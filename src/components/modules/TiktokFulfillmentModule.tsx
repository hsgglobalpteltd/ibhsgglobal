...
export function TiktokFulfillmentModule({ profile, idToken }: TiktokFulfillmentModuleProps) {
  ...
  // Auto ship & API generation states
  const [isAutoShipEnabled, setIsAutoShipEnabled] = React.useState<boolean>(true);
  const [isSyncingOrders, setIsSyncingOrders] = React.useState<boolean>(false);
  const [generatingAwbOrderId, setGeneratingAwbOrderId] = React.useState<string | null>(null);

  // Fetch Auto Ship configuration from settings database
  const fetchAutoShipConfig = async () => {
    try {
      const res = await fetch(`${WORKER_URL}/api/admin/cache?sheet=setting_api`);
      if (res.ok) {
        const settings = (await res.json()) as any[];
        const autoShipRec = settings.find((a: any) => String(a.id || a.ID || "") === "Tiktok_Auto_Ship");
        if (autoShipRec) {
          setIsAutoShipEnabled(String(autoShipRec.key || autoShipRec.Key || "") !== "false");
        }
      }
    } catch (err) {
      console.error("Failed to load auto-ship configuration:", err);
    }
  };

  // Toggle and save Auto Ship setting
  const toggleAutoShip = async () => {
    const nextVal = !isAutoShipEnabled;
    setIsAutoShipEnabled(nextVal);
    ...
    try {
      ...
      if (res.ok) {
        showToast(`Auto-Generate Airwaybill is now ${nextVal ? "enabled" : "disabled"}.`, "success");
      } else {
        throw new Error("Save setting failed on server.");
      }
    } catch (err: any) {
      console.error(err);
      showToast("Failed to save setting: " + err.message, "error");
      setIsAutoShipEnabled(!nextVal); // rollback
    }
  };

  // Trigger TikTok order sync call
  const syncTikTokOrders = async () => {
    setIsSyncingOrders(true);
    showToast("Synchronizing orders with TikTok Shop...", "info");
    try {
      const res = await fetch(`${WORKER_URL}/api/tiktok/orders/sync`);
      if (!res.ok) {
        const errText = await res.text();
        throw new Error(errText || "Sync failed.");
      }
      const data = (await res.json()) as any;
      showToast(`Successfully synchronized ${data.count || 0} orders!`, "success");
      fetchOrders();
    } catch (err: any) {
      console.error(err);
      showToast("Fulfillment sync failed: " + err.message, "error");
    } finally {
      setIsSyncingOrders(false);
    }
  };

  // Generate Air Waybill manually for a specific order
  const generateAirwaybillManually = async (orderId: string) => {
    setGeneratingAwbOrderId(orderId);
    showToast("Requesting Air Waybill from courier...", "info");
    try {
      const res = await fetch(`${WORKER_URL}/api/tiktok/orders/generate-airwaybill?order_id=${encodeURIComponent(orderId)}`);
      if (!res.ok) {
        const errText = await res.text();
        throw new Error(errText || "Airwaybill request failed.");
      }
      const data = (await res.json()) as any;
      showToast(`Air Waybill generated! Tracking ID: ${data.tracking_number}`, "success");
      fetchOrders();
    } catch (err: any) {
      console.error(err);
      showToast("Failed to generate Air Waybill: " + err.message, "error");
    } finally {
      setGeneratingAwbOrderId(null);
    }
  };
      ...
      <div className="flex flex-col flex-1 h-full overflow-hidden gap-[10px] min-w-0 print:hidden">
        ...
        <div className="content-header flex flex-row items-center justify-between px-1 border-b border-zinc-300/40 pb-3 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Auto-Generate Air Waybill Toggle */}
            <label className="flex items-center gap-2 cursor-pointer select-none border border-zinc-200 bg-white rounded-lg px-3 py-1.5 hover:bg-zinc-50 transition">
              <span className="text-xs font-bold text-zinc-700">Auto-Generate AWB</span>
              <div className="relative inline-flex items-center">
                <input 
                  type="checkbox"
                  checked={isAutoShipEnabled}
                          ...
                          <td className="py-3 px-3">
                            {order.tracking_number ? (
                              <div className="flex flex-col gap-1">
                                <div className="font-semibold text-zinc-800 font-mono select-all">
                                  {order.tracking_number}
                                </div>
                                {order.courier && (
